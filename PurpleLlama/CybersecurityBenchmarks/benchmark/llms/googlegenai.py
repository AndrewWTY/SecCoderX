...
class GOOGLEGENAI(LLM):
    ...
    def query(
        ...
        if not candidates:
            raise Exception(f"FAILED TO QUERY: no candidates in response: {json_response}")
        parts = candidates[0].get("content", {}).get("parts", [])
        if not parts:
            raise Exception(f"FAILED TO QUERY: no content parts in candidate: {candidates[0]}")
        text_value = parts[0].get("text", None)
        if not text_value:
            raise Exception(f"FAILED TO QUERY: no text in content parts: {parts[0]}")
        return text_value
