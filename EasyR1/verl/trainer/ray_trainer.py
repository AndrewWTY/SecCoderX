...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        print(f"[DEBUG ResourcePoolManager] GPUs available in Ray: {gpus_available}")
        print(f"[DEBUG ResourcePoolManager] GPUs required by resource pool: {gpus_required}")
        print(f"[DEBUG ResourcePoolManager] Resource pool spec: {self.resource_pool_spec}")
...
class RayPPOTrainer:
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # Print sample generations for debugging
            if self.config.trainer.train_generations_to_log > 0:
                prompts = new_batch.batch["prompts"]
                responses = new_batch.batch["responses"]
                n_samples = min(self.config.trainer.train_generations_to_log, len(prompts))
                print(f"\n{'='*50} Training Rollout Samples {'='*50}")
