...
def main(
    *,
    ...
    parser.add_argument(
        "--judge-batch",
        action="store_true",
        help="Use Google Batch API for judge evaluation (instruct benchmark). Requires --judge-api-key.",
    )

    parser.add_argument(
        "--judge-batch-auto",
        action="store_true",
        help="Skip confirmation prompt and submit batch directly.",
    )

    parser.add_argument(
        "--judge-api-key",
        help="Google API key for batch judge evaluation. Required when --judge-batch is used.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limit number of samples for batch judge evaluation (for debugging).",
    )

    parser.add_argument(
        "--reference-model",
        help="Model name suffix for reference_code field (e.g., 'codellama-7b-instruct' uses 'reference_code_codellama-7b-instruct'). If not specified, uses 'reference_code'.",
    )
    ...
    if args.judge_llm:
        if args.judge_batch:
            # For batch mode, just extract the model name from the spec (PROVIDER::MODEL)
            # without creating the full LLM object (which requires API key in spec)
            parts = args.judge_llm.split("::")
            if len(parts) >= 2:
                judge_model = parts[1]
            else:
                raise ValueError(f"Invalid judge-llm spec for batch mode: {args.judge_llm}. Expected format: PROVIDER::MODEL")
        else:
    ...
    config: BenchmarkConfig = BenchmarkConfig(
        ...
        judge_batch_auto=args.judge_batch_auto,
        judge_api_key=args.judge_api_key,
        judge_batch_limit=args.judge_batch_limit,
